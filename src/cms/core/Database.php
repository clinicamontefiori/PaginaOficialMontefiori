...
class Database
{
    ...
    // Función para ejecutar una consulta SELECT y recuperar una columna específica
    public function selectColumn($table, $column, $where = [], $limit = false, $order = false, $where_mode = "AND") {
        $sql = "SELECT $column FROM $table";

        // Construir la cláusula WHERE si se proporcionan condiciones
        if (!empty($where)) {
            $whereClause = [];
            foreach ($where as $campo => $valor) {
                $valor = $this->mysql->real_escape_string($valor); // Prevenir SQL injection
                $whereClause[] = "$campo = '$valor'";
            }
            $sql .= " WHERE " . implode(" $where_mode ", $whereClause);
        }
        ...
        // Agregar la cláusula LIMIT si se especifica
        if ($limit) {
            $sql .= " LIMIT $limit";
        }

        // Ejecutar la consulta
        $resultados = $this->mysql->query($sql);

        if (!$resultados) {
            die("Error en la consulta: " . $this->mysql->error);
        }

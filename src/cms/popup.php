					...
					<div class="card-body">
						<div class="table-responsive">
							<table id="example2" class="table table-striped table-bordered">
								...
								<?php
								// $searproye = file_get_contents("php/medicos.json");
								// $searproye = json_decode($searproye);
								$searproye = lista_registros_cms('jpopup_general', 'json');
								foreach ( $searproye as $ProSeObj ){

								//$url = str_replace("https://clinicasanfelipe.com/sites/default/", "../assets/img/", $ProSeObj->uri);
								//$url_medico = urls_amigables($ProSeObj->nombre)."-".urls_amigables($ProSeObj->apellido);
								?>
									<tr>
										<td><?php echo $ProSeObj->titulo?></td>
										<td><?php echo $ProSeObj->imgdesktop?></td>
										<td><?php echo $ProSeObj->imgmovil?></td>
										<td><?php echo $ProSeObj->url?></td>
										...
										<td>
											<div class="d-flex order-actions">
												<a href="popup-add?id=<?php echo $ProSeObj->idpopup?>" class=""><i class='bx bxs-edit'></i></a>
												<a href="#" data-id='<?php echo $ProSeObj->idpopup?>' class="enviarPost ms-3"><i class='bx bxs-trash'></i></a>
											</div>
										</td>
									</tr>
								<?php } ?>	
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.enviarPost', async function (e) {
            ...
            try {
				...
				$form.append('<input type="hidden" id="idpopup" name="idpopup" value="' + idpopup + '">');

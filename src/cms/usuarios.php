...
<body>
					...
					<div class="card-body">
						<div class="table-responsive">
							<table id="example2" class="table table-striped table-bordered">
								...
								<?php
								$searproye = lista_registros_cms('jusuarios_general', 'json');
								foreach ( $searproye as $ProSeObj ){
								?>
									<tr>
										<td><?php echo $ProSeObj->usuario?></td>
										<td><?php echo $ProSeObj->email?></td>
										<td><?php echo $ProSeObj->nombre_rol?></td>
										<td><span class="badge bg-success"><?php echo $ProSeObj->google2fa?></span></td>
										...
										<td>
											<div class="d-flex order-actions">
												<a href="usuarios-add?id=<?php echo $ProSeObj->idusuario?>" class="ms-3">
													<i class='bx bxs-edit'></i></a> 
												<a href="#" data-id='<?php echo $ProSeObj->idusuario?>' class="enviarPost ms-3"><i class='bx bxs-trash'></i></a> 
											</div>
										</td>
									</tr>
								<?php } ?>	
    ...
    $(document).ready(function () {
        ...
        //$('.enviarPost').click(async function (e) {
        $(document).on('click', '.enviarPost', async function (e) {
            e.preventDefault();
            
            const $link = $(this);
            const idusuario = $link.data("id");
            ...
            try {
                ...
                const $form = $('<form>', {
                    method: 'POST',
                    action: 'controlador/usuarios.php',
                    target: '_self'
                });

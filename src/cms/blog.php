					...
					<div class="card-body">
						<div class="table-responsive">
							<table id="example2" class="table table-striped table-bordered">
								...
								<?php
								$searproye = lista_registros_cms('jblog_general', 'json');
								foreach ( $searproye as $ProSeObj ){
								?>
									<tr>
										<td><?php echo $ProSeObj->titulo?></td>					
										<td><?php //echo $ProSeObj->especialidad?></td>
										<td><?php if ($ProSeObj->destacado=='on'){?><span class="badge bg-success">Destacado</span><?php } ?></td>
										...
										<td>
											<div class="d-flex order-actions">
												<a href="blog-add?id=<?php echo $ProSeObj->idblog?>" class=""><i class='bx bxs-edit'></i></a>
												<a href="#" data-id='<?php echo $ProSeObj->idblog?>' class="enviarPost ms-3"><i class='bx bxs-trash'></i></a>
											</div>
										</td>
									</tr>
								<?php }  ?>	
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.enviarPost', async function (e) {
            ...
            try {
				...
				$form.append('<input type="hidden" id="idblog" name="idblog" value="' + idblog + '">');

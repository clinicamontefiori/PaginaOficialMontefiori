...
if($_GET){
	...
	$searproye = detalle_registros_cms('jtestimonios', 'json');
	$proceso ='upd';

	foreach ( $searproye as $ProSeObj ){
		if ( $ProSeObj->idtestimonio == $idpage ) {
			$idtestimonio = $ProSeObj->idtestimonio;
			$titulo = $ProSeObj->titulo;
			$bajada = $ProSeObj->bajada;
			$imgdesktop = $ProSeObj->imgdesktop;
			$estado = $ProSeObj->estado;
			$youtube = $ProSeObj->youtube;
...
<body>
	...
	<div class="wrapper">
		...
		<div class="page-wrapper">
			<div class="page-content">
				<!--breadcrumb-->
				<?php echo imprime_breadcrumb('Testimonios'); ?>
				<!--end breadcrumb-->
					...
					<div class="col-xl-6 mx-auto">
						<div class="card">
							<div class="card-header px-4 py-3">
								<h5 class="mb-0">Mantenimiento Testimonios</h5>
							</div>
							<div class="card-body p-4">
								<form class="row g-3 needs-validation" data-sitekey="<?php echo SITE_KEY; ?>"  action="controlador/testimonios" method="post" enctype="multipart/form-data" novalidate="">
									<div class="col-md-12">
										<input type="hidden" name="recaptcha_token" id="recaptcha_token">
										<input type="hidden" id="idtestimonio" name="idtestimonio" value="<?php echo $idtestimonio; ?>">
									...
									<div class="col-md-12">
										<label for="bsValidation13" class="form-label">Testimonio</label>
										<textarea class="form-control" id="bajada" name="bajada" placeholder="Ingresa testimonio" rows="3"><?php echo $bajada?></textarea>
									</div>

									<div class="col-md-12">
										<label for="bsValidation1" class="form-label">Youtube</label>
										<input type="text" class="form-control" id="youtube" name="youtube" placeholder="DM2p-z8M9Vg" value="<?php echo $youtube?>" >
									</div>
									...
									<div class="col-md-12">
										...
										<?php if ($imgdesktop!='') { ?>
												<a target="_blank" href="../uploads/testimonios/<?php echo $imgdesktop;?>">[ver IMG]</a> <a href="#" data-id='<?php echo $idtestimonio?>' data-vatipo='imgdesktop' class="enviarPost ms-3"><i class='bx bxs-trash'></i></a>
										<?php } ?>
										</div>
									</div>
	...
	<script>
		...
		$(document).ready(function () {
            $('.enviarPost').click(function (e) {
            	...
            	var idtestimonio = $(this).data("id");

                $.ajax({
                    type: 'POST',
                    url: 'controlador/testimonios.php', // Reemplaza con la URL de tu script PHP
                    data: { recaptcha_token: token, proceso: proceso, idtestimonio: idtestimonio },
	...
	<script type="text/javascript">
		grecaptcha.ready(function() {
			grecaptcha.execute('<?php echo SITE_KEY; ?>', {action: 'AgregarTestimonio'})
			.then(function(token) {
			  //alert(token)

        ...
        fetch(endpoint)
            ...
            .then(doctors => {
                ...
                const scored = doctors.map(doc => {
                    ...
                    const keys = doc.keywords?.toLowerCase() || "";

                    let score = 0;

                    // Coincidencia exacta en keywords → muy alto
                    if (keys.includes(q)) score += 50;

                    // Coincidencia exacta en especialidad
                    if (spec.includes(q)) score += 25;

                    // Coincidencia exacta en nombre
                    if (name.includes(q)) score += 20;

                    // Palabras separadas (más flexible)
                    q.split(" ").forEach(word => {
                        if (word.length > 2) {
                            if (keys.includes(word)) score += 10;
                            if (spec.includes(word)) score += 6;
                ...
                // Filtrar solo los que tengan un score válido
                const filtered = scored
                    .filter(doc => doc.score > 0)
                    .sort((a, b) => b.score - a.score); // mayor score → más arriba
